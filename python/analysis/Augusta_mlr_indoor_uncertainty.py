...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 25 10:45:13 2020
...
def indoor_mlr_uncertainty(stdev_number,location):
    
    sensor = location
    
    if stdev_number == 1 and sensor['Location'].str.contains('Audubon').any():

        sigma_i_Augusta = 2.45
        sigma_i_BAM = 1.74
    ...
    sensor['lower_uncertainty'] = sensor['PM2_5_corrected']-(sensor['PM2_5_corrected']*((((((sigma_i_Augusta/sensor['PM2_5_corrected'])*100))**2+(((sigma_i_BAM/sensor['PM2_5_corrected'])*100))**2)**0.5)/100))
    sensor['upper_uncertainty'] = sensor['PM2_5_corrected']+(sensor['PM2_5_corrected']*((((((sigma_i_Augusta/sensor['PM2_5_corrected'])*100))**2+(((sigma_i_BAM/sensor['PM2_5_corrected'])*100))**2)**0.5)/100))
